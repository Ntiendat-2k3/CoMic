      ...
      {loading ? (
        ...
        <ul className="grid grid-cols-2 gap-6 lg:grid-cols-3">
          {items.map(({ slug, comic, meta }) => {
            ...
            return (
              <li key={slug} className="overflow-hidden rounded-lg bg-gray-800 shadow-lg hover:shadow-2xl transition-shadow">
                <Link href={`/truyen-tranh/${slug}/${meta}`} className="block">
                  <img src={imgSrc} alt={comic.name} className="h-48 w-full object-cover" />
                </Link>
                <div className="p-4">
