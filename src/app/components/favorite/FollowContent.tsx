...
export default function FollowContent() {
  ...
  if (!isSignedIn) {
    return <div className="p-6 text-center text-lg text-white">Vui lòng đăng nhập để xem truyện theo dõi.</div>
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <h1 className="mb-6 text-xl lg:text-2xl font-bold text-white">Truyện bạn theo dõi</h1>
      ...
      {loading ? (
        ...
        <p className="text-center text-white/70">Bạn chưa theo dõi truyện nào.</p>
      ) : (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {comic.category?.length > 0 && (
                    <p className="mt-1 text-sm text-white/70">{comic.category.map((c) => c.name).join(", ")}</p>
                  )}
                  {lastRead && <p className="mt-1 text-sm text-yellow-300">Đã đọc đến chap {lastRead}</p>}
                </div>
              </li>
            )
          })}
