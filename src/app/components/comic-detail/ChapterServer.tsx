...
const ChapterServer = ({ server, comicSlug, serverIndex }: ChapterServerProps) => {
  return (
    <div key={`server-${serverIndex}-${server.server_name}-${comicSlug}`} className="space-y-4">
      {/* Server Header */}
      <div className="bg-gray-700 rounded-lg p-3">
        <div className="flex items-center justify-between">
          ...
          <div className="bg-gray-600 px-2 py-1 rounded text-xs text-gray-300">{server.server_data.length} chương</div>
        </div>
      </div>

      {/* Chapter Grid - Fixed scrolling */}
